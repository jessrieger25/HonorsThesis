...
class TSNEVisualizations():
    ...
    def run(self, vectors, words, word2int, pic_int, sizes={}, separates=[], keywords={}):
        size_list = list()
        ...
        plt.show()
        fig.savefig('/Users/Jess/PycharmProjects/Honors_Thesis_2/graphics/tsne_' + datetime.utcnow().isoformat('T') + '.png')

        # Help to display later
        # from IPython.display import Image
        # Image('my_figure.png')

        self.scatterplot(vectors[:, 0], vectors[:, 1], x_label='x', y_label='y', title='tsne', sizes=size_list, lists=separates)
    ...
    def scatterplot(self, x_data, y_data, x_label="", y_label="", title="", color="r", yscale_log=False, sizes=[], lists=[]):
        ...
        plt.xlabel('x')
        plt.ylabel('y')
        plt.title('Scatter Plot of Ficino\'s Text')
        # x.legend()
        plt.show()
        # fig.savefig('/Users/Jess/PycharmProjects/Honors_Thesis_2/graphics/tsne_scatter_' + datetime.utcnow().isoformat('T') + '.png')
