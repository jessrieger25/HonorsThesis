    ...
    def average_distances(self, relational_dict, keyword):
        ...
        with open(os.path.abspath("../g_star_graphs/average_dist_" + keyword + '_b1-4_kv3.txt'), 'w') as g_star:
            ...
            for word, count in relational_dict.items():
                ...
                if word != '':
                    print(word)
                    g_star.write('add vertex ' + word.strip() + ' with attributes(size=' + str(abs(count-max)).strip() + ', color=' + self.keyword_manager.category_colors[self.keyword_manager.keywords_list[word.strip()]] + ')\n')